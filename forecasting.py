...
def predict_num_nodes(graph_list : Iterable[ig.Graph], h : int=5, conf_level:Optional[int]=None, conf_lower=90):
    ...
    if (num_graphs < 15):
        ...
        exogenous = statsmodels.tools.tools.add_constant(np.arange(num_graphs).reshape((num_graphs, 1)))
        ...
        prediction = model.predict(h=h, level=[conf_level] if conf_level is not None else None)
    ...
    return new_nodes, conf_lower, conf_upper#{"new_nodes":, "conf_lower":, "conf_upper":}

def predict_existing_nodes_degrees(graph_list  : Iterable[ig.Graph], h : int=5, conf_level:Optional[int]=None):
    num_graphs = len(graph_list)
    ...
    if conf_level is not None:
        rename_dict.update({f"{model_str}-lo-{conf_level}":"conf_low", f"{model_str}-hi-{conf_level}":"conf_high"})
    prediction = model.fit_predict(df=df, id_col="node_index", time_col="time_step", target_col="degree", h=h, level = [conf_level])
...
def predict_total_edges_counts(graph_list  : Iterable[ig.Graph], h : int=5, conf_level:Optional[int]=None):
    ...
    if conf_level is not None:
        results.update({'conf_low':np.ceil(prediction[f"lo-{conf_level}"].iloc[-1]).astype(int), 'conf_high':np.ceil(prediction[f"hi-{conf_level}"].iloc[-1]).astype(int)})
    return results
...
def predict_new_nodes_degrees(graph_list : Iterable[ig.Graph]):
    ...
    for graph_idx in range(1, len(graph_list)):
        ...
        this_mean_new_nodes = np.mean(this_new_nodes_degrees) if len(this_new_nodes_degrees) > 0 else 0
