...
def get_neighbours_of_neighbours(gr: ig.Graph) -> np.ndarray:
    ...
    non = np.vstack(np.nonzero((adj2 - adj1) > 0)).T #neighbours of neighbours, not including existing edges
...
def check_solution_against_constraints(solution_graph:ig.Graph, union_graph: ig.Graph, max_node_degrees: list[int], total_edges_constraint:int) -> bool:
    all_ok = True
    #check total number of edges
    num_solution_edges = solution_graph.ecount()
    if num_solution_edges > int(total_edges_constraint):
        print(f"num solution edges = {num_solution_edges}, total_edges constraint = {total_edges_constraint}, union_graph edges= {union_graph.ecount()}")
        all_ok = False

    #check the degree constraints of each node
    over_degree_capacity = [n for n in range(solution_graph.vcount()) if solution_graph.degree()[n] > int(max_node_degrees[n])]
    if len(over_degree_capacity) > 0:
        for n in over_degree_capacity:
            print(f"degree of Node {n}: {solution_graph.degree()[n]} is larger than its constraint: {int(max_node_degrees[n])}")
            all_ok = False
            continue
    
    if num_solution_edges >= int(total_edges_constraint): return all_ok
    ...
    for n in range(union_graph.vcount()):
        ...
        if solution_degree == degree_constraint: continue
        for e in union_graph.es(_source=n):
            other = e.source if e.target == n else e.target
            if solution_graph.degree()[other] >= int(max_node_degrees[other]): continue
            if solution_graph.are_connected(n, other): continue
            spare_degrees.append((n, other, e))
            print(f"Spare connection from {n}: {solution_degree} < {degree_constraint} to {other}: {solution_graph.degree()[other]} < {int(max_node_degrees[other])} through edge {e.index}")
            all_ok = False
    ...
    def count_triangles(gr : ig.Graph) -> np.ndarray:
        ...
        for triangle in triangles:
            for vertex in triangle:
                counts[vertex] += 1
        return counts
    return sum(count_triangles(gr)) / (gr.vcount() * (gr.vcount()-1) * (gr.vcount()-2) / 6)
...
def evaluation(actual_graph :ig.Graph, predicted_graph : ig.Graph) -> dict:
    ...
    triangle_density_err = abs(triangle_density(predicted_graph) - triangle_density(actual_graph))
    max_degree_err = abs( max(predicted_graph.degree()) / max(actual_graph.degree()) - 1 )

    d = dict(vcount_err = vcount_err,
...
def diff_metrics(act_adj : np.ndarray, pred_adj : np.ndarray) -> dict:
  ...
  if (act_adj.shape != pred_adj.shape):
      raise ValueError("The actual and predicted adjacency lists must have the same shapes")
  n = act_adj.size
...
def eval_metrics2(actual_graph : ig.Graph, predicted_graph : ig.Graph) -> dict:
    ...
    new_vert = None
    node_error = (predicted_graph.vcount() - actual_graph.vcount()) / actual_graph.vcount()
