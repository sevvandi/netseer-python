...
def construct_union_graph(graph_list : list[ig.Graph], num_new_nodes : int, new_nodes_degrees, remove_vertices : Optional[int] = 0, weights_option : int = 2, weights_param = 0, num_most_connected_nodes : int = 10):
    def weights_value(weights_option : int, index : int, num_graphs : int) -> float:
        #different options for assigning weights to each edge
        if weights_option <= 3: return 0
        if weights_option == 4: return 1
        if weights_option == 5: return index + 1
        if weights_option == 6: return 1/(num_graphs - index)
        if weights_option == 7: return 1 if index == num_graphs - 1 else 0
        return 0
    ...
    for graph_idx in range(num_graphs):
        graph_list[graph_idx].es[this_weights_key(graph_idx)] = weights_value(weights_option, graph_idx, num_graphs)
    ...
    for graph_idx in range(num_graphs):
        ...
        np.nan_to_num(this_weights, copy=False, nan=0.0) #this is for edges that exist somewhere in the union graph, but not in this graph
    ...
    if (num_new_nodes > 0):
        ...
        most_connected_nodes = np.argpartition(existing_node_degrees, kth=-num_most_connected_nodes)[-num_most_connected_nodes:] # attach potential edges to this number of nodes in the union graph, with the highest degrees
        ...
        new_node_edges = [(old_node, new_node) for old_node in most_connected_nodes for new_node in new_nodes]
        if weights_option in [1, 2]:
            ...
            existing_nodes_quantiles = existing_node_degrees/(sum(existing_node_degrees))
            existing_nodes_weights = np.quantile(a=weights, q = existing_nodes_quantiles)
        
        union_graph.add_edges(new_node_edges, attributes = {WEIGHTS: new_nodes_edges_weights})#
...
def setup_lp_solver(union_graph : ig.Graph, max_node_degrees : list[int], total_edges : int) -> pulp.LpProblem:
    ...
    variables = [pulp.LpVariable(str(e_idx), cat="Binary") for e_idx in range(len(weights))]
    ...
    for node_idx in range(union_graph.vcount()):
        ...
        this_expression = pulp.LpAffineExpression([(variables[edge], 1) for edge in this_edges])
        this_constraint = pulp.LpConstraint(e=this_expression, sense=pulp.LpConstraintLE, rhs=max_node_degrees[node_idx], name=str(node_idx))
    ...
    total_edges_constraint = pulp.LpConstraint(e=total_edges_expression, sense=pulp.LpConstraintLE, rhs=total_edges, name="total_edges")
...
def run_lp_solver(problem : pulp.LpProblem) -> list[int]:
    ...
    if status != pulp.constants.LpSolutionOptimal:
        print(f"Error running solver - {status=} - {pulp.constants.LpSolution[status]}")#
    vars = problem.variables()
    solution_edge_idxs = [int(edge.name) for edge in vars if edge.varValue > 0]
    return solution_edge_idxs

def make_solution_graph(union_graph : ig.Graph, solution_edge_idxs : list[int]) -> ig.Graph:
    #solution_edges = []
    return union_graph.subgraph_edges(edges=solution_edge_idxs, delete_vertices=False)

def predict_graph_from_forecasts(graph_list : list[ig.Graph], new_nodes : int, existing_nodes_degrees, new_nodes_degrees : int, total_edges, conf_nodes : Optional[int] = None, conf_degree : Optional[int] = None, weights_option : int = 1) -> ig.Graph:
    ...
    union_graph = construct_union_graph(graph_list, new_nodes, new_nodes_degrees, weights_option = weights_option)
    ...
    solution_ok = utils.check_solution_against_constraints(solution_graph, union_graph, max_node_degrees, total_edges)
